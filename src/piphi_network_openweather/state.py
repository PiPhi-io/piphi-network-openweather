...
def _split_config(config: DeviceConfig) -> tuple[dict[str, Any], dict[str, str]]:
...
def make_entry(config: DeviceConfig) -> dict[str, Any]:
    ...
    public_config, _ = _split_config(config)
    return {
        **identity,
        "host": config.host,
        "alias": config.alias,
...
async def apply_config(config: DeviceConfig) -> None:
    ...
    _, secrets = _split_config(config)
    if secrets:
        credential_store[config.id] = secrets
...
async def remove_config(config_id: str) -> bool:
    ...
    credential_store.pop(config_id, None)
    if entry is None:
        return False
    append_runtime_event(
...
def _register_automation_actions() -> None:
    for command_name, command_definition in commands.items():
        def handler(request, *, _command_name=command_name):
            ...
            device_id = str(request.device_id or target.get("device_id") or "openweather-location")
